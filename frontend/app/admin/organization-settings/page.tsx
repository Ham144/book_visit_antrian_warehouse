...
const OrganizationSettingsPage = () => {
  ...
  const { mutateAsync: handleSubmit, isPending } = useMutation({
    mutationKey: ["oraganization-settings", "update"],
    mutationFn: async () =>
      await OrganizationApi.updateOrganizationSettings(settingsData),
    onSuccess: (res) => {
  ...
  useEffect(() => {
    setSettingsData(myOrganizationSettings);
            ...
            <div className="p-6 md:p-8 space-y-8 max-h-[calc(100vh-300px)] overflow-auto">
              ...
              <div className="border-t border-teal-100 pt-8 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {allMenuAndFeatures.map((feature) => (
                    <div
                      key={feature.id}
                      className="flex items-center p-3 border border-teal-200 rounded-lg"
                    >
                      <input
                        type="checkbox"
            ...
            <div className="bg-teal-50 px-6 py-4 border-t border-teal-100">
              <div className="flex flex-col md:flex-row md:items-center justify-between">
                <div className="text-sm text-teal-700 mb-3 md:mb-0">
                  <p>Organisasi: {settingsData?.name || "Belum diatur"}</p>
                </div>
                <button
                  type="submit"
                  disabled={isPending}
